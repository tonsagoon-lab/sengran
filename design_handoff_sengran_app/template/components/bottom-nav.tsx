// components/bottom-nav.tsx — 5 tabs with protruding center FAB
"use client";
...
const TABS: { id: TabId; href: string; icon: React.ComponentType<{ className?: string }>; label: string; primary?: boolean }[] = [
...
export function BottomNav({ active }: { active: TabId }) {
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-5 border-t border-neutral-800 bg-neutral-900 pb-2 pt-1.5">
      {TABS.map((t) => {
        const Icon = t.icon;
        ...
        if (t.primary) {
          return (
            ...
            >
              <span className="-mt-3 grid size-11 place-items-center rounded-full bg-orange-500 text-white shadow-[0_6px_18px_rgb(249_115_22_/_0.5)] transition-transform hover:scale-105">
        ...
        return (
          <Link
            ...
            className={cn(
              ...
              on ? "text-orange-500" : "text-white/55 hover:text-white",
            )}
          >
            <Icon className="size-[22px]" />
